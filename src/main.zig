...
const demo_scene = @import("demo/scene.zig");
...
pub fn main() !void {
    ...
    const font = try text_renderer.loadFont(demo_scene.font_path, demo_scene.font_size_px);
    ...
    while (!glfw.shouldClose(window)) {
        ...
        text_renderer.begin();
        try scene.draw(&text_renderer, font);
        text_renderer.flush(frame.cmd, scene.projection(w, h), viewport);
