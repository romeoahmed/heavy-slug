...
pub fn main() !void {
    ...
    while (!glfw.shouldClose(window)) {
        ...
        try text_renderer.drawText(font_large, "heavy-slug",
            ...
            .{ 1.0, 1.0, 1.0, 1.0 });
        try text_renderer.drawText(font_medium, "GPU Text Rendering",
            pga.Motor.fromTranslation(w * 0.5 - 160, h - 110),
            .{ 0.6, 0.7, 0.9, 1.0 });
        ...
        try text_renderer.drawText(font_medium, "Spinning!",
            rot_motor,
            .{ 1.0, 0.8, 0.2, 1.0 });
        ...
        try text_renderer.drawText(font_small, "orbiting",
            pga.Motor.fromTranslation(orbit_x, orbit_y),
            .{ 0.4, 1.0, 0.6, 1.0 });

        // === Bouncing text ===
        const bounce_y = h * 0.2 + 30.0 * @abs(@sin(t * 3.0));
        try text_renderer.drawText(font_medium, "Bounce!",
            pga.Motor.fromTranslation(60, bounce_y),
            .{ 1.0, 0.4, 0.5, 1.0 });
        ...
        try text_renderer.drawText(font_small, "Slug algorithm: exact quadratic Bezier coverage",
            pga.Motor.fromTranslation(40, 80),
            .{ r, g, b, 1.0 });

        // === FPS counter (top-right area) ===
        try text_renderer.drawText(font_small, fps_text[0..fps_len],
            pga.Motor.fromTranslation(w - 100, h - 30),
            .{ 0.5, 1.0, 0.5, 1.0 });
