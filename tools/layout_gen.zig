...
//!
//! Fields named with a leading `_` receive zero defaults so callers do not
//! have to initialize padding fields explicitly.
...
fn fieldsEqual(a: []const FieldLayout, b: []const FieldLayout) bool {
    ...
    for (a, b) |af, bf| {
        ...
        if (af.offset != bf.offset or af.size != bf.size) return false;
...
fn appendUniqueStruct(
    ...
    result: *std.ArrayList(StructLayout),
    layout: StructLayout,
) !void {
    ...
    try result.append(allocator, layout);
}
...
fn extractStruct(
    ...
    var cursor: u32 = 0;
    for (field_list.items) |f| {
        if (f.offset < cursor) return error.OverlappingField;
        cursor = f.offset + f.size;
    }
    if (total_size < cursor) return error.StructSizeTooSmall;
...
/// Parse reflection JSON. Free the returned layouts with `freeStructs`.
fn parseReflection(allocator: std.mem.Allocator, json_bytes: []const u8) ![]StructLayout {
    ...
    for (params_arr.items) |param_val| {
        ...
        if (std.mem.eql(u8, kind_str, "resource")) {
            ...
            try appendUniqueStruct(allocator, &result, s);
...
fn emitZig(writer: *std.Io.Writer, structs: []const StructLayout) !void {
    ...
    for (structs) |s| {
        ...
        for (s.fields) |f| {
            try writer.print("    try std.testing.expectEqual(@as(usize, {d}), @offsetOf({s}, \"{s}\"));\n", .{ f.offset, s.name, f.name });
        }
        try writer.writeAll("}\n");
    }
}
...
test "parseReflection rejects conflicting duplicate struct layouts" {
    ...
    try std.testing.expectError(error.ConflictingStructLayout, parseReflection(std.testing.allocator, json));
}
...
test "parseReflection rejects overlapping fields" {
    ...
    try std.testing.expectError(error.OverlappingField, parseReflection(std.testing.allocator, json));
}
